...
设计文档另约定可选同级 ``workspace/``（Claw/CLI 源文件池）及配置文件
``workspace/mmem-workspace.json``（不入 ``repo``），见 ``docs/memory-repo-extended-layout.md``。
...
def seed_default_workspace_template(user_uuid: str, agent_name: str) -> bool:
    """
    若包内存在 ``mindmemory_client.agent.<name>.workspace`` 模板目录，
    且用户 ``workspace/`` 下尚无 ``mmem-workspace.json``，则复制模板文件（不覆盖已有文件）。

    内置提供 **BT-7274** 的 identity / soul 样板与 ``mmem-workspace.json``。
    """
    from importlib.resources import files

    from mindmemory_client.sync_manifest import WORKSPACE_CONFIG_FILENAME

    dest = resolve_workspace_dir_for_user_agent(user_uuid, agent_name)
    if (dest / WORKSPACE_CONFIG_FILENAME).is_file():
        return False
    try:
        root = files("mindmemory_client").joinpath("agent", agent_name, "workspace")
        if not root.is_dir():
            return False
    except (NotImplementedError, FileNotFoundError, ModuleNotFoundError, OSError):
        return False
    ...
    return (dest / WORKSPACE_CONFIG_FILENAME).is_file()
...
def write_agent_config(
    ...
    *,
    ...
    try:
        seed_default_workspace_template(user_uuid, agent_name)
    except Exception as e:
        _logger.debug("seed_default_workspace_template: %s", e)
    return p
...
def ensure_default_agent_workspace(cfg: MindMemoryClientConfig) -> dict[str, Any]:
    ...
    if agent_config_path(uid, name).is_file():
        ...
        try:
            seed_default_workspace_template(uid, name)
        except Exception as e:
